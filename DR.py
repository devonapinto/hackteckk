...
def DR():
    ...
    def circle(morph_img,rgb_img,max_val):
        ...
        #cv2.imshow('masked image',morph_img)
        #cv2.waitKey(0)
        ...
        for c in cnts:
            ...
            if len(approx) > 5 and area > 40 and area < 5000:
        ...
        cv2.waitKey()'''
        if circle_count<4:
            cursor.execute("INSERT INTO result (disorders,final) VALUES ('dr','false');")
            conn.commit()
        else:
            cursor.execute("INSERT INTO result (disorders,final) VALUES ('dr','true');")
            conn.commit()
            save_path = "C:\\Users\\maqwi\\Desktop\\hack\\dr\\final.png"
            cv2.imwrite(save_path, rgb_image)
            f=open("dr\\dr.txt","w")
            f.write("regions detected : "+str(circle_count)+"\n")
        cursor.close()
        conn.close()
    ...
    circle(morph_img,rgb_image,k_max_val)
    pass
